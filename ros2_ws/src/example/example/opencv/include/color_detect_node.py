    ...
    def main(self):
        while self.running:
            ...
            for i in self.target_colors:
                if self.detect_type[i] == 'line' and i != '':
                    ...
                    for roi in self.line_roi:
                        ...
                        mask = cv2.inRange(blob, tuple(self.lab_data['color_range_list'][i]['min']), tuple(self.lab_data['color_range_list'][i]['max']))  # 二值化
                        ...
                        if max_contour_area is not None:
                            ...
                            cv2.drawContours(result_image, [box], -1, self.range_bgr[i], 2)  # 画出四个点组成的矩形
                            ...
                            cv2.circle(result_image, (int(line_center_x), int(line_center_y)), 5, self.range_bgr[i], -1)  # 画出中心点
            ...
            if areaMaxContour_rect is not None:
                ...
                cv2.drawContours(result_image, [box], -1, self.range_bgr[color_area_max_rect], 2)  # 画出四个点组成的矩形
            ...
            if areaMaxContour_circle is not None:
                ...
                radius = int(radius)
                cv2.circle(result_image, (x, y), radius, self.range_bgr[color_area_max_circle], 2)  # 画圆
